...
export const updateArray = (data, rowIndex, cellIndex, newVal, real_years, country, arrayIndex) => {
    ...
    const offset = DataManager.countryData[country].offset;
    const mapping = DataManager.countryData[country].mapping;
    ...
    while(currYear < totalYears) {
        if (arrayIndex === 3) { //pnl updates have little care for real years
            ...
            updateReal2(updatedData, currYear, offset, mapping);
...
const updateReal = (array, currYear, offset, mapping) => {
    updateRevenueReal(array, currYear, offset);
...
/* OFFSET AND MAPPING METHODS */

const updateRevenueReal = (array, year, offset) => {
  if(offset[0]===0) {
  ...
  } else { // this means that revenue is the product of volume and price, the next two rows
    array[0][year] = array[1][year] * array[2][year];
    array[3][year] = (year === 0 || array[0][year - 1] === 0) ? 0 : (array[0][year] / array[0][year - 1]) - 1;
  }
};

const updateRevenueDependency = (array, year, index, offset, mapping, real) => {
  const loc = mapping[index];
  const perc = mapping[index+1];
  if(offset[loc] === 0) {
    if(real) { // gross profit typed in, get percent
      array[perc][year] = array[0][year]===0 ? 0 : array[loc][year]/array[0][year];
    } else { //retrieve gp from the percent
      array[loc][year] = array[perc][year] * array[0][year];
    }
  } else {
    let sum = 0;
    for(let i = loc+1; i < perc; i++) {
      sum += array[i][year];
    }
    array[loc][year] = sum;
    array[perc][year] = array[0][year]===0 ? 0 : array[loc][year]/array[0][year];
...
const updateEbit = (array, currYear, mapping) => {
  array[mapping[12]][currYear] = array[mapping[2]][currYear] - array[mapping[4]][currYear] - array[mapping[6]][currYear] - array[mapping[8]][currYear] - array[mapping[10]][currYear];
  array[mapping[13]][currYear] = array[0][currYear]===0 ? 0 : array[mapping[12]][currYear]/array[0][currYear]
};

const updateEbitda = (array, year, mapping) => {
  array[mapping[16]][year] = array[mapping[12]][year] + array[mapping[14]][year];
...
const updateReal2 = (array, currYear, offset, mapping) => {
  updateRevenueReal(array, currYear, offset);
  updateRevenueDependency(array, currYear, 2, offset, mapping, true); //Gross Profit
  updateRevenueDependency(array, currYear, 4, offset, mapping, true); // A&C
  updateRevenueDependency(array, currYear, 6, offset, mapping, true); // Distribution
  updateRevenueDependency(array, currYear, 8, offset, mapping, true); // Ovh
  updateRevenueDependency(array, currYear, 10, offset, mapping, true); // OIE
  updateEbit(array, currYear, mapping); // D&A
