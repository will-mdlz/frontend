...
import { dis_labels as ROW_LABELS } from '../../../constants';

const DisTemplate = ({segKey, handleDataChanged}) => {
    ...
    useEffect(() => {
      ...
      if(!hasCombinedData.current) {
        const combinedData = dataManagerInstance.getDis(segKey);
  ...
  const total_years = dataManagerInstance.rawdata["DIS"][segKey][0].length;
  const COLUMN_LABELS = generateColumnLabels(dataManagerInstance.input["DIS"][segKey]['startingyear'], total_years);
