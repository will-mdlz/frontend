...
import { ncore_cons_labels as ROW_LABELS } from '../../../constants';

const NcoreCons = ({dataChanged}) => {
    ...
    useEffect(() => {
      ...
      //if(!hasCombinedData.current) {
        ...
        keys.forEach((num) => {
            dataManagerInstance.calcNcore(num);
        })
        dataManagerInstance.calcConsolidatedNcore();
        const combinedData = dataManagerInstance.getNcore("CONS");
  ...
  let perc_rows = [1,6,8,10,12,16,18]
  let dollar_rows = [0,2,5,9,13,19]
