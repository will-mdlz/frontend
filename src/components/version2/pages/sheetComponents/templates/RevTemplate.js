...
import { rev_labels as ROW_LABELS } from '../../../constants';

const RevTemplate = ({segKey, handleDataChanged}) => {
    ...
    useEffect(() => {
      ...
      if(!hasCombinedData.current) {
        const combinedData = dataManagerInstance.getRev(segKey);
  ...
  let perc_rows = [1,3,5,7,9]
  let dollar_rows = [0,8]
  let editable = [0,2,4,6]
