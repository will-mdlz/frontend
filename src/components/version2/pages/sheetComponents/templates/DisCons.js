...
import { dis_cons_labels as ROW_LABELS } from '../../../constants';

const DisCons = ({dataChanged}) => {
    ...
    useEffect(() => {
      ...
      //if(!hasCombinedData.current) {
        ...
        keys.forEach((num) => {
            dataManagerInstance.calcDis(num);
        })
        dataManagerInstance.calcConsolidatedDis();
        const combinedData = dataManagerInstance.getDis("CONS");
