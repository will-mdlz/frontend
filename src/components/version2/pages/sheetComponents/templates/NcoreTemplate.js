...
import { ncore_labels as ROW_LABELS } from '../../../constants';

const NcoreTemplate = ({segKey, handleDataChanged}) => {
    ...
    useEffect(() => {
      ...
      if(!hasCombinedData.current) {
        const combinedData = dataManagerInstance.getNcore(segKey);
  ...
  const total_years = dataManagerInstance.rawdata["NCORE"][segKey][0].length;
  const COLUMN_LABELS = generateColumnLabels(dataManagerInstance.input["NCORE"][segKey]['startingyear'], total_years);
  ...
  const handleBlur = (row, col) => (event) => {
    ...
    if(currVal==="") {
      ...
      // Update things
      dataManagerInstance.calcNcore(segKey)
      setVal('');
      handleDataChanged();
    }
    setEditMode([])
