...
import { cost_cons_labels as ROW_LABELS } from '../../../constants';

const CostCons = ({dataChanged}) => {
    ...
    useEffect(() => {
      ...
      //if(!hasCombinedData.current) {
        ...
        keys.forEach((num) => {
            dataManagerInstance.calcCost(num);
        })
        dataManagerInstance.calcConsolidatedCost();
        const combinedData = dataManagerInstance.getCost("CONS");
